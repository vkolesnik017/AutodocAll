...
public class DataBase {
    ...
    List<String> getRouteListForMain(String shop) throws SQLException {
        ...
        Connection conn = coonectionDB("routes_prf");
        ArrayList<String> route = new ArrayList<>();
        String query = "SELECT " + shop + " FROM autodoc.routes_prf where id = 1";
        try {
    ...
    List<String> getRouteListByRouteName(String shop, String routeName) throws SQLException {
        ...
        Connection conn = coonectionDB("routes_prf");
        ArrayList<String> route = new ArrayList<>();
        String query = "SELECT " + shop + " FROM autodoc.routes_prf where route_name = '" + routeName + "'";
        try {
            statement = conn.createStatement();
            ResultSet resultSet = statement.executeQuery(query);
            int columns = resultSet.getMetaData().getColumnCount();
    ...
    public String getKba(String shop) throws SQLException {
        Statement statement = null;
        Connection conn = coonectionDB("kba_prf");
        String kba = null;
        String query = "SELECT " + shop + " FROM autodoc.kba_prf where id = 1";
        try {
            statement = conn.createStatement();
            ResultSet resultSet = statement.executeQuery(query);
            while (resultSet.next()) {
