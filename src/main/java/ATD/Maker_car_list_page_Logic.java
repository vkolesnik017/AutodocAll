...
public class Maker_car_list_page_Logic extends Maker_car_list_page {
    ...
    public Maker_car_list_page_Logic compareDisplayOrderOfChildCategoriesWithFile(String file, List<String> notUsedChild) {
        ...
        for (int i = 0; i < titleOfParentCategories().size() - 1; i++) {
            if (notUsedChild.contains(titleOfParentCategories().get(i).getText())) {
                continue;
            }
            ...
            System.out.println();
            int currentParentCategory = parentsFromFile.indexOf(titleOfParentCategories().get(i).getText());
            int nextParentCategory = parentsFromFile.indexOf(titleOfParentCategories().get(i + 1).getText());
            System.out.println();
            for (int j = currentParentCategory; j < nextParentCategory; j++) {
                childFromFile.add(childFromExcel.get(j));
            }
            Assert.assertEquals(childFromFront, childFromFile, String.format("list of child not equals in %s - parent category", titleOfParentCategories().get(i).getText()));
